...
3.  **AI適合判定・マッチング (ローカル / Modal AIインフラ)**
    *   `bge-base-ja-v1.5` による768次元セマンティック検索（ローカル ONNX / WASM 完結）。大規模処理時は Modal GPU にオフロード。
    *   団体のプロファイルと助成金要件の17項目3段階ハイブリッド判定（確定ルール / 書類突合 / LLM根拠引用）。
    *   BM25 + pgvector RRF ハイブリッド検索 + `bge-reranker-base` (ONNX) 二次リランク。
