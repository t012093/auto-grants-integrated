...
| **Data_Store_Knowledge** (データストア & ナレッジ化) | 永続化・ベクトル化・構造化・リアルタイム配信 | **Supabase (PostgreSQL 15 + pgvector)** に全テーブルを一元管理。GraphRAG で政策・施策を知識グラフ化。**@huggingface/transformers (ONNX/WASM)** によるローカル Embedding 生成（**bge-base-ja-v1.5 768次元**）→ Supabase pgvector に保存。BM25 + pgvector RRF + **bge-reranker-base (ONNX)** 二次リランク。Supabase Realtime でリアルタイム通知。 |
