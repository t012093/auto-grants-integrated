...
| **Data_Store_Knowledge** (データストア & ナレッジ化) | 永続化・ベクトル化・構造化・リアルタイム配信 | PostgreSQL 15 + pgvector に全テーブルを一元管理。GraphRAG で政策・施策を知識グラフ化。**@huggingface/transformers (Transformers.js / WASM / ONNX) + PGlite** によるオンデバイス/ローカルEmbedding（**bge-base-ja-v1.5 768次元**）と、大規模処理時の **Modal GPU** オフロードによるハイブリッド構成。BM25 + pgvector RRF + **bge-reranker-base (ONNX)** 二次リランク。Supabase Realtime でリアルタイム通知。 |
