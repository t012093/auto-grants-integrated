...
## 10. ハーネス安全制御 & マルチフォーマット出力 (`grant_form_filler`)

### ノンインタラクティブ & 安全自動補完原則
* **基本動作 (ノンインタラクティブ完走)**: 自動化パイプラインを中断させないため、データ未登録時（過去採択事例0件や希望経費未設定）は標準テンプレート/デフォルト配分で自動補完して生成を最後まで完走させる。
* **自動補完注記**: 自動補完された箇所には `💡 [自動補完注記: 〜]` を明記し、人間による推稿・校正を助ける。
* **厳格モード (`--strict`)**: フラグ指定時のみ、データ不足時に自動補完を行わずエラー中断する。

### 算術・ルール検算 (Harness Guard)
* 生成原稿内の経費合計値が Solver の確定額と一致しているか、必須6大セクションが存在するか算術・構造検証。
* バリデーションエラー発生時はファイル出力をストップし、エラーログを表示。

### 出力フォーマット
* **Office ドキュメント (`officecli`)**:
  - **パターン A (フリーフォーマット/デフォルト)**: Markdown原稿から美しい段落・見出し構成の `.docx` を新規構築。
  - **パターン B (`--template-docx` 指定時)**: 指定様式 `.docx` の `{{事業目的}}` 等を `officecli merge` でプレースホルダー置換。
  - **Excel (`--with-budget-xlsx`)**: 5 カラム構成（経費区分, 項目名, 希望額, 助成対象決定額, 充当理由）で `.xlsx` エクスポート。
