...
*   **メモリ見積もり**: 1024次元ベクトル × 10,000件 ≈ 約 40MB（量子化適用時）。上限 256MB を超過した場合は古いインデックスを自動 evict。
...
### E. Modal AI プリウォーム (Pre-warming) 仕様
* **クライアント側トリガー**: アプリ起動時（ダッシュボード読み込み完了直後）に、フロントエンドからバックグラウンドで `/api/v1/dashboard/agent-status` に対して非ブロッキングのステータス確認リクエストを非同期で1回送信する。
* **目的**: ユーザーが実際に AI 関連のアクション（助成金マッチングや提案書生成）を実行する前に、バックグラウンドで GPU コンテナのスピンアップを開始させ、初回実行時のレイテンシを最小化する。
