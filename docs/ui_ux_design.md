...
7. **🌐 政策・助成金接続フロー (Policy Connection Flow)**
   * **概要**: 自治体の総合計画（Why） ── 施策（What） ── 助成金（How） ── 実行プロジェクト（Action）の繋がりを整理した2D階層型フロー画面。
   * **UX**: 4つの階層カラムカードが左から右へBezier曲線で接続。任意のカードクリック時に、その関連する関係パス（Path）のみが発光し、無関係なカードは半透明にフェードアウト。情報のトレーサビリティをシンプルに体感させます。また、バックエンドでの「横方向の関連度（並列マッチング度）」もデータ構造として考慮します（実装フェーズは後期に設定）。
...
## 5. Modal AIスピンアップ・プリウォーム (Pre-warming)

コールドスタート時の待ち時間（最大3分）によるユーザー体験の低下を緩和するため、以下のプリウォーム仕様をフロントエンドおよびAPI層に実装します。

* **UIトリガー**: アプリ起動時（ダッシュボード読み込み完了直後）に、バックグラウンドから非同期で `/api/v1/dashboard/agent-status` に対してステータス確認リクエストを1回だけ自動送信します。
* **非ブロッキング**: このリクエストは非同期（Fire-and-Forget）で実行され、フロントエンドの主要なUIレンダリングやその他の初期化処理を一切ブロックしません。
* **効果**: ユーザーが実際に「助成金マッチング」や「提案書ジェネレーター」にアクセスし、検索や生成の実行ボタンを押す前に、バックグラウンドで GPU インフラの自動スピンアップ（起動）プロセスを開始させ、初回の待ち時間を大幅に削減します。
```
