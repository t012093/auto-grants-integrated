...
|---|---|---|
| `npo_profiles` | 団体名、活動概要、実績 (ACT 40万/足立区 30万等) | 団体基本情報から概略文を自動補完 |
| `grants` / `extract_pdf` | **公募要領の構造化 5 大要件** (審査基準・配点・公募趣旨・事業期間・対象外条件・必要書類) | タイトル・公募内容よりテキスト抽出 |
| `grant_expense_rules` + `npo_expense_preferences` | 最適化済み経費ポートフォリオ (対象外経費 100% 物理排除) | デフォルト配分 (人件費50%, システム費30%, 広報費20%) で補完 |
| `grant_past_awards` (分析済み) | 勝因パターン・KPI 相場・タイトル提案 | 同ジャンル標準成果目標 (参加者数・満足度等) で補完 |

### Step 2: 申請書 6 大セクション自動起草 (要件連動)
公募要領の抽出要件および原文引用を根拠として主要セクションを自動起草します。

1. **事業の背景・社会的課題**: 公募要領の「審査基準・重点項目」を踏まえ、地域統計や当事者ニーズを提示
2. **事業目的**: 抽出した「公募趣旨・目指す社会像」と団体ミッションの合致点を明示
3. **実施計画・月別スケジュール**: `project_period` から取得した正確な事業月数に合わせて月別活動計画を展開
4. **実施体制**: 団体メンバー・自社過去採択実績 (ACT/足立区等)・連携パートナーの役割分担表
5. **期待される成果 (KPI)**: 審査配点の高い目標指標に優先度を合わせた定量成果設定
6. **経費明細 (自動計算済み)**: Solver 確定済みの経費ポートフォリオを対象外経費 100% 排除でそのまま転記
